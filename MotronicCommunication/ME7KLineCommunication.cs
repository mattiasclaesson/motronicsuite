...
namespace MotronicCommunication
{
    ...
    public class ME7KLineCommunication : ICommunication
    {
        ...
        //ME7: I suggest you try K-line 5-baud init to address 33 to see if it is alive.
        // Then you can try K-line boot mode and send 00 to see if you receive C5.
                ...
                try
                {
                    ...
                    switch (_state)
                    {
                        case CommunicationState.Start:
                            ...
                            //_port.BaudRate = 5;
                            // we need to send 0x33 at 5 baud
                            //_port.Write("\x33");
                            _port.BreakState = true;    // stop bit
                            Thread.Sleep(200);
                            _port.BreakState = false;    // bit 0
                            Thread.Sleep(200);
                            _port.BreakState = false;    // bit 1
                            Thread.Sleep(200);
                            _port.BreakState = true;    // bit 2
                            Thread.Sleep(200);
                            _port.BreakState = true;    // bit 3
                            Thread.Sleep(200);
                            _port.BreakState = false;   // bit 4
                            Thread.Sleep(200);
                            _port.BreakState = false;    // bit 5
                            Thread.Sleep(200);
                            _port.BreakState = true;    // bit 6
                            Thread.Sleep(200);
                            _port.BreakState = true;    // bit 7
                            Thread.Sleep(200);
                            _port.BreakState = false;   // start bit
                            Thread.Sleep(200);
                            //Thread.Sleep(2000);
                            //while (_port.BytesToWrite > 0) Thread.Sleep(0);
                            _state = CommunicationState.WaitForKeywords;
                            _port.BaudRate = 10400;
                            _timeout = 0;
                            break;
