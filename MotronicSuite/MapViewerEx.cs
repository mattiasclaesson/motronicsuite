...
//                simpleButton9.Enabled = m_isOpenSoftware;
        ...
        {
            InitializeComponent();
            simpleButton2.Enabled = false; // data not changed initially
            simpleButton3.Enabled = false;
...
/*            chartControl1.Series[0].ArgumentDataMember = "X";*/
        ...
        public override void ShowTable(int tablewidth, bool issixteenbits)
        {
            ...
            if (m_map_length != 0 && m_map_name != string.Empty)
            {
                ...
                if (indicatorwidth > 0)
                {
                    if (Y_axis_name.Contains("RPM") || Y_axis_name.Contains("Unknown") || Y_axis_name.Contains("Volt") || Y_axis_name.Contains("[ms]") || indicatorwidth >= 16) indicatorwidth = 0x1E;
            ...
            if (m_TableWidth > 1)
            {
                ...
                for (int t = y_axisvalues.Length - 1; t >= 0; t--)
                {
                    ...
                    scaleConfiguratorX.Labels.Add(GetYaxisValue(t, yvalue));
                }
                ...
                for (int t = 0; t < x_axisvalues.Length; t++)
                {
                    ...
                    scaleConfiguratorY.Labels.Add(GetXaxisValue(t, xvalue));
                }
            ...
            if (m_TableWidth == m_map_length)
            {
                ...
                trackBarControl1.Properties.Minimum = 0;
                trackBarControl1.Properties.Maximum = y_axisvalues.Length - 1;
                labelControl8.Text = X_axis_name + " values";
                ...
                else
                {
                    for (int t = 0; t < m_map_length; t++)
                    {
                        ...
                        if (x_axisvalues.Length > valcount)
                        {
                            //yval = Convert.ToDouble(GetXaxisValue((int)valcount));//yval = Convert.ToDouble((int)x_axisvalues.GetValue((int)valcount));
                            yval = Convert.ToDouble(GetXaxisValueAsFloat((int)valcount, ""));
                            //yval = GetXaxisValue(t, yval.ToString());
                        }
                        //Console.WriteLine("Adding: " + yval.ToString() + " " + value.ToString());
                        chartdt.Rows.Add(yval, value);
                        valcount++;
                    }
                }
        ...
        private void Init2dChart()
        {
            ...
            for (int i = 0; i < x_axisvalues.Length; i++)
            {
                float val = GetXaxisValueAsFloat(i, "");
                //Console.WriteLine("Adding: " + val.ToString());
                surface.XValues.Add(/*x_axisvalues.GetValue(i)*/ val);
            }
        ...
        private float GetXaxisValueAsFloat(int idx, string currValue)
        {
            float retval = 0F;
            try
            {
            ...
            // we only get here is we found nothing
            retval = (float)x_axisvalues[idx];
            return retval;
        }
        ...
        private void UpdateChartControlSlice(byte[] data)
        {
            ...
            try
            {
                ...
                if (m_issixteenbit)
                {
                    ...
                    {
                        double yval = valcount;
                        double value = Convert.ToDouble(data.GetValue(offsetinmap + (t * 2))) * 256;
                        value += Convert.ToDouble(data.GetValue(offsetinmap + (t * 2) + 1));
                        if (value > 32000)
                        {
                            value = 65536 - value;
                            value = -value;
                        ...
                        if (y_axisvalues.Length > valcount)
                        {
                            ...
                            yval = GetXaxisValueAsFloat((int)valcount, "");
                        }
                        chartdt.Rows.Add(yval, value);
                        valcount++;
                    }
                ...
                else
                {
                    for (int t = 0; t <= (numberofrows - 1); t++)
                    {
                        ...
                        {
                            yval = GetXaxisValueAsFloat((int)valcount, "");
                            //yval = Convert.ToDouble((int)x_axisvalues.GetValue((int)valcount));
                        }
...
//                simpleButton3.Enabled = false;
        ...
        private string GetYaxisValue(int idx, string currValue)
        {
            ...
            if (y_axisSymbol == null) return retval;
        ...
        {
            string retval = currValue.ToString();
            if (x_axisSymbol == null) return retval;
...
/*                                if(value < 0)
        ...
        private bool AxisHasLeadingAxis(AxisCollection axis, int address, out int axisaddress)
        {
            bool retval = false;
            axisaddress = 0;
            foreach (AxisHelper ah in m_axisCollection)
            {
        ...
        {
            // get the corresponding axis symbolnames here.
            string x = string.Empty;
            string y = string.Empty;

            foreach (SymbolHelper sh in m_SymbolCollection)
            {
                if (sh.Varname == m_map_name)
                {
                    // get the axis
                    foreach (AxisHelper ah in m_axisCollection)
                    {
                        ...
                        if (endaddress == sh.Flash_start_address)
                        {
                            ...
                            int newaddress = 0;
                            if (AxisHasLeadingAxis(m_axisCollection, ah.Addressinfile, out newaddress))
                            {
            ...
            if (x != string.Empty)
            {
                ...
                editXaxisSymbolToolStripMenuItem.Text = "Edit x-axis (" + y + ")";
            }
            else
            {
            ...
            if (y != string.Empty)
            {
                ...
                editYaxisSymbolToolStripMenuItem.Text = "Edit y-axis (" + x + ")";
            }
            else
            {
