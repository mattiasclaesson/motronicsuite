...
namespace MotronicSuite
{
    public partial class ctrlDisassembler : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private string _motronicFile;

        public string MotronicFile
        {
            get { return _motronicFile; }
            set { _motronicFile = value; }
        }
                ...
                if (words.Count > 0)
                {
                    if (words[0].Word.StartsWith("0x"))
                    {
                        ...
                        if (address > 0)
                        {
                            ...
                            //TODO: Offset 0 for 8051 family? <GS-15022011>
        ...
        public string DisassembleFile()
        {
            bool _skipDisassembly = false;
            
            string outputfile = Path.Combine(Path.GetDirectoryName(_motronicFile), Path.GetFileNameWithoutExtension(_motronicFile) + ".asm");
            ...
            if (!_skipDisassembly)
            {
                ...
                if (disasm.DisassembleFileSim8051(_motronicFile) != string.Empty)
                {
                    LoadFile(outputfile);
                }
            }
            ...
            string copyFile = _motronicFile + DateTime.Now.Ticks.ToString();
            File.Copy(_motronicFile, copyFile);
            LoadBinaryFile(copyFile, new SymbolCollection());
            return outputfile;
        }
